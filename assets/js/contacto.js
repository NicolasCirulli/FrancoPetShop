...
submit.addEventListener('click', (e) => {
    ...
    (fullName.value && pruebaemail(email.value) == "email valido") ?
        gato.checked || perro.checked ?
            ...
            fullName.value = '',email.value = '',phone.value = '',mensaje.value = '',gato.checked = false,perro.checked = false) 
            :  (divAlerta.classList = 'alert alert-success d-flex justify-content-between',
            spanMensaje.innerText = 'Debe seleccionar al menos una mascota') :(divAlerta.classList = 'alert alert-success d-flex justify-content-between',
            spanMensaje.innerText = 'Email no valido') ;
